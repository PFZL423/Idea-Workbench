            ...
            "final_ideas": [
                {
                    ...
                    "proposal_summary": "构建一个最小 diagnostic benchmark，检验 WAM 的 prediction accuracy 是否真的对应可控 action effects。",
                    "research_question": "When does WAM prediction accuracy fail to imply controllable action effects in contact-rich manipulation?",
                    "target_problem": "现有 WAM 评估容易只看预测误差，无法说明模型是否捕捉了 action 对关键 contact state 的可控影响。",
                    ...
                    "proposed_method": "构造 prediction-matched WAMs，并加入 action intervention probes 来测量可控因素是否被表示。",
                    "mechanism_design": "固定 prediction error 相近的模型组，比较 intervention probe、affordance metric 和 downstream success 的关系。",
                    "training_signal": "主要训练信号仍是 next-state prediction，诊断信号来自 intervention probe 与 downstream action success 的相关性。",
                    "evaluation_protocol": "在 contact-rich toy tasks 中控制 prediction error，报告 probe score、affordance score、action success 和 failure cases。",
                    "expected_contribution": "给出 WAM 评估中 prediction-control gap 的可复现实证边界。",
                    "why_this_is_better_than_initial_version": "把原本宽泛的 WAM architecture idea 收窄成一个可证伪的 diagnostic contribution。",
                    ...
                    "evidence_basis": ["Prior review suggests prediction accuracy alone is an insufficient novelty claim."],
                    "open_assumptions": ["There exists a measurable gap between prediction-matched models on action success."],
